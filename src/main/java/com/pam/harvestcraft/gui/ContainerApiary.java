...
public class ContainerApiary extends Container {
	...
	public ContainerApiary(InventoryPlayer inv,
			TileEntityApiary tileEntityApiary) {
		this.apiary = tileEntityApiary;

		for (int x = 0; x < 6; ++x) {
			for (int y = 0; y < 3; ++y) {
				addSlotToContainer(new SlotPamResult(tileEntityApiary,
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getStack() != null) {
			...
			if (index >= 19
					? (slotStack.getItem() == ItemRegistry.queenbeeItem
