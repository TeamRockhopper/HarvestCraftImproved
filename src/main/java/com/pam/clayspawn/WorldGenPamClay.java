...
public class WorldGenPamClay implements IWorldGenerator {
	...
	private void GenerateOverworld(Random random, int x, int z, World world) {
		if (clayspawn.enableClaySpawn) {
			this.addOreSpawn(Blocks.CLAY, world, random, x, z, 4, 6,
					clayspawn.clayRarity, 0, 128);
		}
		if (clayspawn.enableHardenedClaySpawn) {
			this.addOreSpawn(Blocks.HARDENED_CLAY, world, random, x, z, 4, 6,
					clayspawn.hardenedClayRarity, 0, 128);
		}
	}
	...
	public void addOreSpawn(Block block, World world, Random random,
		...
		WorldGenMinable minable = new WorldGenMinable(block.getDefaultState(),
				6);
