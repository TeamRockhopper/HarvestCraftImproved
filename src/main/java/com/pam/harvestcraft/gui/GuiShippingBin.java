...
public class GuiShippingBin extends GuiContainer {
	private static final ResourceLocation gui = new ResourceLocation(
			"harvestcraft:textures/gui/shippingbin.png");

	private int itemNum;

	private final TileEntityShippingBin tileEntityShippingBin;

	public GuiShippingBin(InventoryPlayer inventoryplayer,
			TileEntityShippingBin tileEntityShippingBin) {
	...
	public void initGui() {
		...
		final GuiButton right = new GuiButton(1, posX + 16, posY - 21, 15, 20,
				">");
		final GuiButton button_buy = new GuiButton(2, posX, posY + 1, 55, 20,
				"Sell");
		...
		if (guibutton.id == 2) {
			...
			if (buySlot != null) {
				...
				if (buySlot.getItem() == data.getCurrency().getItem()) {
					if (buySlot.getItemDamage() == data.getCurrency()
							.getItemDamage()) {
						int price = data.getPrice();
						if (buySlot.stackSize == price) {
							PacketHandler.network.sendToServer(
									new MessageShippingBinBuy(this.itemNum,
											this.tileEntityShippingBin.getPos()
													.getX(),
											this.tileEntityShippingBin.getPos()
													.getY(),
											this.tileEntityShippingBin.getPos()
						...
						} else if (buySlot.stackSize > price
								&& buySlot.stackSize > 1) {
							PacketHandler.network.sendToServer(
									new MessageShippingBinBuy(this.itemNum,
											this.tileEntityShippingBin.getPos()
	...
	public void onGuiClosed() {
		PacketHandler.network.sendToServer(new MessageShippingBinClosed(
				this.tileEntityShippingBin.getPos().getX(),
				this.tileEntityShippingBin.getPos().getY(),
				this.tileEntityShippingBin.getPos().getZ()));
		super.onGuiClosed();
	}

	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		this.fontRendererObj.drawString("Inventory", 8, (ySize - 96) + 13,
				4210752);
		...
		int price = data.getPrice();
		this.fontRendererObj.drawString("x" + Integer.toString(price), 116, 20,
				0);
