...
public class BeehiveWorldGen implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world,
		...
		final Biome biome = world
				.getBiomeForCoordsBody(new BlockPos(chunkX, 0, chunkZ));
	...
	private static void tryGenerateBeehives(World world, Random random,
		...
		for (int i = 0; i < HarvestCraft.config.beehiveRarity; ++i) {
			...
			world.setBlockState(variableBlockPos,
					BlockRegistry.beehive.getDefaultState());
		}
	}
	...
	private static int getHeightBelowLeaves(World world, int posX, int posY,
			int posZ) {
		final BlockPos.MutableBlockPos variableBlockPos = new BlockPos.MutableBlockPos();
		variableBlockPos.setPos(posX, posY, posZ);

		// Starting from a known leaves block, descend until air block is
		// reached.
		// If another block is reached (non-leaves, non-air), abort and return
		// -1.
		for (int y = posY, sealevel = world.getSeaLevel(); y >= sealevel; --y) {
			variableBlockPos.setY(y);
			final Block block = world.getBlockState(variableBlockPos)
					.getBlock();

			if (block == Blocks.LEAVES || block == Blocks.LEAVES2)
				continue;

			if (world.isAirBlock(variableBlockPos))
				return y;
