...
public class BlockPamSapling extends BlockBush implements IGrowable {
	...
	public BlockPamSapling(String name, SaplingType saplingType) {
		...
		leavesState = Blocks.LEAVES.getDefaultState()
				.withProperty(BlockOldLeaf.VARIANT, planks)
	...
	private void validatePosition(World world, BlockPos pos,
			IBlockState state) {
	...
	private boolean isSuitableSoilBlock(Block soilBlock) {
		return soilBlock == Blocks.GRASS || soilBlock == Blocks.DIRT
				|| soilBlock == Blocks.FARMLAND;
	}
	...
	private void generateTree(World worldIn, BlockPos pos, IBlockState state,
			Random rand) {
		...
		if (getFruit() instanceof BlockPamFruit) {
			if (!new FruitTreeGen(5, logState, leavesState, false, fruitState)
					.generate(worldIn, rand, pos)) {
				worldIn.setBlockState(pos, state); // Re-add the sapling if the
													// tree failed to grow
			}
		} else if (getFruit() instanceof BlockPamFruitLog) {
			if (!new LogFruitTreeGen(5, logState, leavesState, fruitState)
					.generate(worldIn, rand, pos)) {
	...
	public void worldGenTrees(World world, BlockPos pos) {
		for (int tries = 0; tries < rarity; tries++) {
			int posX = pos.getX() + world.rand.nextInt(8)
					- world.rand.nextInt(8);
			int posY = pos.getY() + world.rand.nextInt(4)
					- world.rand.nextInt(4);
			int posZ = pos.getZ() + world.rand.nextInt(8)
					- world.rand.nextInt(8);
			...
			if (getFruit() instanceof BlockPamFruit) {
				new FruitTreeGen(5, logState, leavesState, false, fruitState)
						.generate(world, world.rand, newPos);
			} else if (getFruit() instanceof BlockPamFruitLog) {
				new LogFruitTreeGen(5, logState, leavesState, fruitState)
						.generate(world, world.rand, newPos);
			}
		}
	}
	...
	@Override
	public IBlockState getPlant(net.minecraft.world.IBlockAccess world,
			BlockPos pos) {
	...
	public void setFruit(@Nonnull Block fruit) {
		...
		FMLLog.severe("Given fruit block %s is invalid.",
				fruit.getUnlocalizedName());
	}

	public Block getFruit() {
		if (fruit == null) {
			FMLLog.bigWarning("Fruit for sapling %s not found.",
					getUnlocalizedName());
			return null;
		}
