...
public class TileEntityApiary extends TileEntityLockable
	...
	private List<ItemStack> apiaryItemStacks = new ArrayList<ItemStack>();
	// .<ItemStack> withSize(19, ItemStack.EMPTY);
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		for (int i = 0; i < tagList.tagCount(); ++i) {
			...
			if (slot < 0 || slot >= inventory.length)
				continue;
			ItemStack stack = ItemStack.func_77949_a(stackTag);
			inventory[slot] = stack;
			// new ItemStack(stackTag);
		}
	...
	public void update() {
				...
				if (runTime > 0) {
					...
					if (queenBee != null) {
						if (queenBee.getItem().getContainerItem() != null) {
							queenBee = new ItemStack(
									queenBee.getItem().setFull3D());
						} else {
							if (queenBee.stackSize > 0) {
								queenBee.stackSize = (queenBee.stackSize - 0);
							}
						}
		...
		if (needsUpdate) {
			markDirty();
			world.notifyBlockUpdate(pos, world.getBlockState(getPos()),
	...
	private boolean canRun() {
		if (inventory[18] != null) {
			if (inventory[18].getItem() == ItemRegistry.queenbeeItem
					&& inventory[18].getItemDamage() != inventory[18]
							.getMaxDamage()) {
				return true;
			}
		}
		...
		for (int offsetX = -radius; offsetX <= radius; ++offsetX) {
			for (int offsetZ = -radius; offsetZ <= radius; ++offsetZ) {
				if (offsetX * offsetX + offsetZ * offsetZ > radius * radius
						...
						|| offsetX == -radius - 1 && offsetZ == radius - 1)
					continue;
				final Block blockAtCoords = world.getBlockState(
						new BlockPos(varX + offsetX, varY, varZ + offsetZ))
						.getBlock();
				if (blockAtCoords instanceof BlockFlower
						|| blockAtCoords instanceof BlockCrops
