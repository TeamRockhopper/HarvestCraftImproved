...
public class TileEntityGroundTrap extends TileEntity
			...
			for (int offsetZ = -radius; offsetZ <= radius; ++offsetZ) {
				if (offsetX * offsetX + offsetZ * offsetZ <= radius * radius
					...
					final Block blockAtCoords = world.getBlockState(
							...
							.getBlock();
					if (blockAtCoords instanceof BlockDirt
							|| blockAtCoords instanceof BlockGrass) {
						count++;
					}
				}
			}
	...
	private boolean canRun() {
		if (this.inventory[18] != null) {
			if (this.inventory[18].getItem() == ItemRegistry.grainbaitItem
					|| this.inventory[18]
							.getItem() == ItemRegistry.fruitbaitItem
					|| this.inventory[18]
