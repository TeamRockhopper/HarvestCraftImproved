...
public class ContainerWaterTrap extends Container {
	...
	public ContainerWaterTrap(InventoryPlayer inv,
			TileEntityWaterTrap TileEntityWaterTrap) {
		this.watertrap = TileEntityWaterTrap;

		for (int x = 0; x < 6; ++x) {
			for (int y = 0; y < 3; ++y) {
				addSlotToContainer(new SlotPamResult(TileEntityWaterTrap,
						y + x * 3, 62 + 18 * x, 17 + 18 * y));
			}
		}

		addSlotToContainer(
				new SlotPamWaterTrap(TileEntityWaterTrap, 18, 26, 35));
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getStack() != null) {
			...
			if (index >= 19
					? (slotStack.getItem() == ItemRegistry.fishtrapbaitItem
							? !mergeItemStack(slotStack, 18, 19, false)
							: (index >= 19 && index < 46
									? !mergeItemStack(slotStack, 46, 55, false)
									: index >= 46 && index < 55
											&& !mergeItemStack(slotStack, 19,
													46, false)))
			...
			slot.func_82870_a(playerIn, slotStack);
		}
