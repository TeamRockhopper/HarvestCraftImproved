...
public class BlockPamFruit extends Block implements IGrowable, PamCropGrowable {
	...
	public BlockPamFruit(BlockPamSapling sapling, Item fruit) {
		...
		this.setDefaultState(
				this.blockState.getBaseState().withProperty(AGE, 0));
	...
	public BlockPamSapling getSapling() {
		if (sapling == null) {
			FMLLog.bigWarning("Cannot get sapling for fruit %s.",
					getUnlocalizedName());
		}
	...
	@Override
	public float getBlockHardness(IBlockState blockState, World worldIn,
			BlockPos pos) {
	...
	public String getStageId(int stage) {
		if (BASE_STAGE_ID == null) {
			BASE_STAGE_ID = getUnlocalizedName().replaceFirst("pam", "")
					.replaceFirst("tile.", "").toLowerCase() + "_stage";
		}
