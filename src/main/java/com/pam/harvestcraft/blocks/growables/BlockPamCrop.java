...
public class BlockPamCrop extends BlockCrops
	...
	private static final PropertyInteger AGE = PropertyInteger.create("age", 0,
			MATURE_AGE);
	...
	public BlockPamCrop(String registerName, String name) {
		...
		this.setDefaultState(
				blockState.getBaseState().withProperty(getAgeProperty(), 0));
	}

	public String getStageId(int stage) {
		if (BASE_STAGE_ID == null) {
			BASE_STAGE_ID = registerName.replaceFirst("pam", "").replace("Crop",
					"") + "_stage";
		}
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source,
			BlockPos pos) {
		// CROPS_AABB is based on an age range from 0 to 7. Times two should fix
		// that issue.
	...
	public Item getSeed() {
		if (seed == null) {
			FMLLog.bigWarning("No seed have been set up for %s.",
					getUnlocalizedName());
			return new Item();
		}
	...
	public Item getCrop() {
		if (food == null) {
			FMLLog.bigWarning("No food has been set up for %s.",
					getUnlocalizedName());
			return new Item();
		}
			...
			if (currentGrowthLevel < getMatureAge()) {
				...
				if (ForgeHooks.onCropsGrowPre(worldIn, pos, state,
					...
					worldIn.setBlockState(pos,
							this.getStateFromMeta(currentGrowthLevel + 1), 2);
	...
	public void dropBlockAsItemWithChance(World worldIn, BlockPos pos,
		...
		if (!worldIn.isRemote && !worldIn.restoringBlockSnapshots) {
			final List<ItemStack> items = getDrops(worldIn, pos, state,
					fortune);
	...
	public boolean equals(Object obj) {
		return (obj instanceof BlockPamCrop
				&& registerName.equals(((BlockPamCrop) obj).registerName));
	}
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos,
		...
		final Random rand = world instanceof World ? ((World) world).rand
				: new Random();
