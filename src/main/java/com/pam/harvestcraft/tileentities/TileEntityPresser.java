...
public class TileEntityPresser extends TileEntity
	...
	private boolean canRun() {
		...
		final ItemStack[] results = PresserRecipes
				.getPressingResult(inventory[0]);
		if (results == null)
			return false;

		if (inventory[1] != null) {
			if (!inventory[1].isItemEqual(results[0]))
				return false;
			if (inventory[1].stackSize + results[0].stackSize > inventory[1]
					.getMaxStackSize())
				return false;
		}

		if (results[1] != null && inventory[2] != null) {
			if (!inventory[2].isItemEqual(results[1]))
				return false;
			if (inventory[2].stackSize + results[1].stackSize > inventory[2]
					.getMaxStackSize())
				return false;
		}
	...
	private void pressComb() {
		...
		} else if (inventory[1].stackSize + results[0].stackSize <= results[0]
				.getMaxStackSize()) {
		...
		inventory[0].stackSize = (inventory[0].stackSize - 1);
